    ...
    stages {
        ...
        stage('Check for Changes') {
            steps {
                script {
                    def changedFiles = sh(script: 'git diff --name-only origin/${env.BRANCH_NAME}', returnStdout: true).trim().split('\n')
                    boolean configChanged = changedFiles.any { it.endsWith('.yaml') || it.endsWith('.ini') || it.endsWith('.json') || it.contains('config/') }

                    if (changedFiles) {
                        echo "Changes detected in the following files:\n${changedFiles.join('\n')}"
                        env.HAS_CHANGES = 'true'
                        env.CONFIG_CHANGED = configChanged ? 'true' : 'false'
                    } else {
                        echo "No changes detected."
                        env.HAS_CHANGES = 'false'
                        env.CONFIG_CHANGED = 'false'
            ...
            steps {
                withCredentials([string(credentialsId: 'github-pat-token', variable: 'GITHUB_TOKEN')]) {
                    script {
                        ...
                        if [ -z "\$PR_EXISTS" ]; then
                            echo "No existing pull request found. Creating a new one."
                            gh pr create --title "Automated PR from ${env.BRANCH_NAME}" --body "Automated pull request triggered by changes in branch ${env.BRANCH_NAME}." --base "${env.BASE_BRANCH}" --head "${env.BRANCH_NAME}" || exit 1
                        else
                            echo "Pull request #\$PR_EXISTS already exists for branch ${env.BRANCH_NAME} targeting ${env.BASE_BRANCH}."
                        fi
                        """
        ...
        stage('Build') {
            ...
            steps {
                echo "Building on branch: ${env.BRANCH_NAME}"
                // Add build commands here, e.g., mvn clean install, npm install, etc.
            }
        }
